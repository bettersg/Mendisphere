...
const headerSizes = [
  "2xl", // H1 will be '2xl' size
  "2xl", // H2 will be 'xl' size
  // Add more header levels and their sizes as needed
];
...
export const ProfileContent: React.FC = () => {
  const options: HTMLReactParserOptions = {
    replace: (node: any) => {
      if (node.type === "tag" && node.name.startsWith("h1")) {
        return (
          <Heading as={node.name} size="xl" color="blue">
  ...
  return <Box textAlign="left">{parse(htmlContent, options)}</Box>;
};
